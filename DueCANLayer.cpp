...
extern int TimerActivity_CAN0;
extern int TimerActivity_CAN1;
...
// nTxMailboxes should not exceed 4.
//
// Returns CAN_OK/CAN_ERROR
//
byte canInit(byte cPort, long lBaudrate, int nTxMailboxes)
{
  // Declarations
  ...
  if(cRetCode == CAN_OK)
  {
    if(cPort == 0)
    {
      Can0.setNumTXBoxes(nTxMailboxes);
      Can0.watchFor();
    }
    else
    {
      Can1.setNumTXBoxes(nTxMailboxes);
      Can1.watchFor();
    }
  }// end if
...
byte canTx(byte cPort, long lMsgID, bool bExtendedFormat, byte* cData, byte cDataLen)
{
  ...
  {   
    ...
    TimerActivity_CAN0 = ACTIVITY_PULSE;
    
  }// end if
  else
  {
    cRetCode = Can1.sendFrame(frameCANMsg);
    TimerActivity_CAN1 = ACTIVITY_PULSE;
